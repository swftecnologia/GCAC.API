...
{
    /// <summary>
    /// Controlador para tratar os tipos de contato do participante
    /// </summary>
    [ApiController]
    ...
    public class TipoContatoController : ControllerBase
    {
        ...
        /// </summary>
        /// <param name="tipoContatoServico">Serviços da entidade TipoContato</param>
        public TipoContatoController(ITipoContatoServico tipoContatoServico)
        {
        ...
        {
            return await _tipoContatoServico.SelecionarTodos();
        }
        ...
        [ApiConventionMethod(typeof(DefaultApiConventions), nameof(DefaultApiConventions.Get))]
        public async Task<ActionResult<TipoContato>> SelecionarPorId(long id)
        {
            var item = await _tipoContatoServico.SelecionarPorId(id);
        ...
        /// </summary>
        /// <param name="itemDTO">Novo tipo de contato do participante a ser criado</param>
        /// <returns>Quantidade de registros afetados pela operação solicitada</returns>
        [HttpPost]
        [Route("inserir")]
        [ApiConventionMethod(typeof(DefaultApiConventions), nameof(DefaultApiConventions.Post))]
        public async Task<ActionResult<TipoContato>> Inserir(TipoContatoDTO itemDTO)
        {
            if (await _tipoContatoServico.ExistePorDescricao(itemDTO.Descricao))
            {
            ...
            await _tipoContatoServico.Inserir(item);
            itemDTO = item.AsDTO();
        ...
        /// </summary>
        /// <param name="itemDTO">Tipo de contato do participante a ser atualizado</param>
        /// <returns>Quantidade de registros afetados pela operação solicitada</returns>
        [HttpPut]
        [Route("atualizar")]
        [ApiConventionMethod(typeof(DefaultApiConventions), nameof(DefaultApiConventions.Put))]
        public async Task<IActionResult> Atualizar(TipoContatoDTO itemDTO)
        {
            if (await _tipoContatoServico.ExistePorDescricao(itemDTO.Descricao, (long)itemDTO.Id))
            {
                return BadRequest("Não foi possível realizar a solicitação: Grau da entidade do participante já cadastrado.");
            }

            var item = itemDTO.AsEntitie();
            await _tipoContatoServico.Atualizar(item);
        ...
        public async Task<ActionResult<TipoContato>> Excluir(long id)
        {
            var item = await _tipoContatoServico.SelecionarPorId(id);
            ...
            await _tipoContatoServico.Excluir(item);
            return Ok("Solicitação realizada com sucesso.");
        }
    }
}